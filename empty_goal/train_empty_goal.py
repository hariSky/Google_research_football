...
while not target_reached and iters < max_iters:
    ...
    for itr in range(ppo_steps):
        ...
        #if state[87+7]!=0:
        #	reward=reward+(state[87+1]*0.1+0.1) +(0.1-abs(state[87+2]*0.1)) 
        ...
        state = observation
        #print("state shape : {}".format(type(state)))
        #z=K.expand_dims(state, 0)
        #print("z shape : {}".format(type(z)))
        #state_input=tf.convert_to_tensor(np.expand_dims(state, axis=0))
        state_input=np.expand_dims(state, axis=0)
        #print("nz shape : {}".format(nz.shape))  
        if done:
            env.reset()
    ...
    if iters % 80 == 0:
	    ...
	    if avg_reward > best_reward:
	        ...
	        model_actor.save('model_actor_empty_goal_{}_{}.hdf5'.format(iters, avg_reward))
	        model_critic.save('model_critic_empty_goal_{}_{}.hdf5'.format(iters, avg_reward))
	        best_reward = avg_reward
	    if best_reward > 0.9 or iters > max_iters:
	        target_reached = True
	    #for now just save them anyway just to save the model for test
	    model_actor.save('model_actor_empty_goal{}.hdf5'.format(iters),save_format='tf')
	    model_critic.save('model_critic_empty_goal{}.hdf5'.format(iters))
    iters += 1
    env.reset()
