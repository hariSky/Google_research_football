...
if image_based:
    env = football_env.create_environment(env_name='11_vs_11_easy_stochastic', representation='pixels', render=True)
else:
    env = football_env.create_environment(env_name='11_vs_11_easy_stochastic', representation='simple115', render=False)
...
if read_model==1:
	...
	model_actor = tf.keras.models.load_model("model_actor_11_vs_11_easy_stochastic80.hdf5",custom_objects={'loss': ppo_loss(oldpolicy_probs, advantages, rewards, values)})
	model_critic = load_model("model_critic_11_vs_11_easy_stochastic80.hdf5", compile=True)
...
while not target_reached and iters < max_iters:
    ...
    for itr in range(ppo_steps):
        ...
        reward=reward+(state[87+1]*0.1) +(0.1-abs(state[87+2]*0.1)) 
    ...
    #critic_loss = model_critic.fit([np.array(states)], [np.reshape(np.array(returns), newshape=(-1, 1))], shuffle=True, epochs=8,
                                   #verbose=True, callbacks=[tensor_board])

    if iters % 2000 == 0 and iters!=0:
	    avg_reward = np.mean([test_reward() for _ in range(5)])
	    print('total test reward=' + str(avg_reward))
	    if avg_reward > best_reward:
	        print('best reward=' + str(avg_reward))
	        model_actor.save('model_actor_11_vs_11_easy_stochastic_{}_{}.hdf5'.format(iters, avg_reward))
	        model_critic.save('model_critic_11_vs_11_easy_stochastic_{}_{}.hdf5'.format(iters, avg_reward))
	        best_reward = avg_reward
	    if best_reward > 0.9 or iters > max_iters:
	        target_reached = True
	    
	    
	    model_actor.save('model_actor_11_vs_11_easy_stochastic{}.hdf5'.format(iters),save_format='tf')
	    model_critic.save('model_critic_11_vs_11_easy_stochastic{}.hdf5'.format(iters))
    iters += 1
    env.reset()
