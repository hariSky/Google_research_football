...
if read_model==1:
	...
	model_critic = load_model("model_critic_11_vs_11_easy_stochastic1000.hdf5", compile=True)
	    ...
	    model_actor = get_model_actor_simple(input_dims=state_dims, output_dims=n_actions, n_actions=n_actions)
...
while not target_reached and iters < max_iters:
    ...
    for itr in range(ppo_steps):
        ...
        reward=reward+(state[87+1]*0.1) +(0.5-abs(state[87+2]*0.1)) 
    ...
    critic_loss = model_critic.fit(np.array(states), np.array(np.reshape(rewards, newshape=(-1, 1))), shuffle=True, epochs=8,
                                   verbose=True, callbacks=[tensor_board])

    if iters % 500 == 0 and iters!=0:
